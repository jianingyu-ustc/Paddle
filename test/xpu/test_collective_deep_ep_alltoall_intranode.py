...
def is_deep_ep_supported():
    if (
        ...
        or paddle.device.xpu.device_count() < 8
        or True
    ):
        return False
    return True
...
def bench(fn, num_warmups: int = 20, num_tests: int = 30, post_fn=None):
    ...
    # Testing
    start_events = [
        paddle.device.xpu.Event(enable_timing=True) for _ in range(num_tests)
    ]
    end_events = [
        paddle.device.xpu.Event(enable_timing=True) for _ in range(num_tests)
    ]
...
def inplace_unique(x: paddle.Tensor, num_slots: int):
    ...
    # There is a bug when stable is False on XPU, so set stable to True
    sorted_bin_idx = paddle.argsort(
        bin_count,
        axis=-1,
        descending=True,
        stable=True,
...
def test_main(
    ...
    assert num_experts % num_ranks == 0 and (
        num_local_ranks == 8 or num_local_ranks == 4
    )
    ...
    # On XPU, since the logic of get_dispatch_layout is actually implemented inside the dispatch operation, this function is effectively a no-op here.
        ...
        for async_mode in (False, True):
            for current_x in (x_pure_rand, x):
                for with_topk in (False, True):
                    ...
                    if not with_topk:
                        for key in ['topk_idx', 'topk_weights']:
                            dispatch_args.pop(key, None)
    ...
    # Performance testing is temporarily skipped.
...
class TestCollectiveDeepEPAllToAllIntranode(TestDistBase):
    def init_env(self):
        ...
        # pipe_dim * model_dim * sharding_dim = device_count
        # expert_dim = model_dim * sharding_dim
        dims = [1, 1, 1, 1, 1, 8, 8]
